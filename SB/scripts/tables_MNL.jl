...
if(results_NYC)
    ...
    list_n_nodes_BnC_SHS   = zeros(nβ, nS)
    ...
    for iβ ∈ 1:nβ
        ...
        for iS ∈ 1:nS
            ...
            df_SHS_βS = filter(row -> (row.P1_auto), df_βS)
            ...
            list_H_hat[iβ,iS]     = mean(df_SHS_βS.H)
            list_n_submodular_cuts[iβ,iS] = mean(df_SHS_βS.n_submodular_cuts)
            list_P[iβ,iS]         = mean(df_SHS_βS.P)
            list_P1[iβ,iS]        = mean(df_SHS_βS.P1)
            list_δ[iβ,iS]         = mean(df_SHS_βS.δ)
            tlim_reached_SHS[iβ,iS] = size(filter(row -> row.time_exact ≥ row.limit_time, df_SHS_βS))[1]
            ...
            for iα ∈ 1:nα
                ...
                for ib ∈ 1:nb
                    ...
                    df_SHS_βSαb = filter(row -> (row.P1_auto), df_βSαb)
                    list_Z_SIM_αb[iβ,iS,iα,ib]     = mean(df_SHS_βSαb.Z_opt)
                    list_Z_MNL_SIM_αb[iβ,iS,iα,ib] = mean(df_SHS_βSαb.Z_opt_stoch)
    ...
    println("\n.......................... SHS ..........................")
    println("time_exact_SHS = ",round.(time_exact_SHS, digits=2))
    println("time_total_SHS = ",round.(time_total_SHS, digits=2))
    println("tlim_reached_SHS = ",round.(tlim_reached_SHS, digits=2))
    ...
    println("list_n_nodes_BnC_SHS = ",round.(list_n_nodes_BnC_SHS, digits=1))
...
if(results_OUR)
    ...
    list_n_nodes_BnC_SHS   = zeros(nD, nβ, nS)
    ...
    for iD ∈ 1:nD
        ...
        for iβ ∈ 1:nβ
            ...
            for iS ∈ 1:nS
                ...
                df_SHS_DβS = filter(row -> (row.P1_auto), df_DβS)
                ...
                list_H_hat[iD,iβ,iS]     = mean(df_SHS_DβS.H)
                list_n_submodular_cuts[iD,iβ,iS] = mean(df_SHS_DβS.n_submodular_cuts)
                list_P[iD,iβ,iS]         = mean(df_SHS_DβS.P)
                list_P1[iD,iβ,iS]        = mean(df_SHS_DβS.P1)
                list_δ[iD,iβ,iS]         = mean(df_SHS_DβS.δ)
                tlim_reached_SHS[iD,iβ,iS] = size(filter(row -> row.time_exact ≥ row.limit_time, df_SHS_DβS))[1]
    ...
    for iD ∈ 1:nD
        ...
        println("\n.......................... SHS ..........................")
        println("time_exact_SHS         = ",round.(time_exact_SHS[iD,:,:], digits=3))
        println("time_total_SHS         = ",round.(time_total_SHS[iD,:,:], digits=3))
        println("tlim_reached_SHS       = ",round.(tlim_reached_SHS[iD,:,:]))
        ...
        println("list_n_nodes_BnC_SHS   = ",round.(list_n_nodes_BnC_SHS[iD,:,:], digits=1))
