...
include("../source/MNL_functions.jl")
...
list_n_nodes_BnC_SHS   = zeros(nβ, nN, nS)
...
for iβ ∈ 1:nβ
    ...
    for iN ∈ 1:nN
        ...
        df_SHS_βN = filter(row -> (row.P1_auto), df_βN)
        ...
        df_SHS_βN_opt = filter(row -> row.is_optimal, df_SHS_βN)
        ...
        for iS ∈ 1:nS
            ...
            list_H_hat[iβ,iN,iS]     = mean(df_SHS_βNS.H)
            list_n_submodular_cuts[iβ,iN,iS] = mean(df_SHS_βNS.n_submodular_cuts)
            list_P[iβ,iN,iS]         = mean(df_SHS_βNS.P)
            list_P1[iβ,iN,iS]        = mean(df_SHS_βNS.P1)
            list_δ[iβ,iN,iS]         = mean(df_SHS_βNS.δ)
            tlim_reached_SHS[iβ,iN,iS] = size(filter(row -> row.time_exact ≥ row.limit_time, df_SHS_βNS))[1]
            ...
            for ib ∈ 1:nb
                ...
                df_SHS_βNSb = filter(row -> (row.P1_auto), df_βNSb)
                list_Z_MNL_SIM_b[iβ,iN,iS,ib] = 100*mean(df_SHS_βNSb.Z_opt_stoch)/N #objective value stored in % of capture
                list_Z_SIM_b[iβ,iN,iS,ib] = 100*mean(df_SHS_βNSb.Z_opt)/N #objective value stored in % of capture

                #estimate the value of the SIM solution for the MMNL instance
                n_rep=0
                for x_opt_str ∈ df_SHS_βNSb.x_opt
...
println("\n.......................... SHS ..........................")
println("time_exact_SHS         = ",round.(time_exact_SHS, digits=3))
println("time_total_SHS         = ",round.(time_total_SHS, digits=3))
