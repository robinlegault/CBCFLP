...
##########################################################################################################
#################### Append a row containing the fields of Stats_SHS to an Excel file ####################
##########################################################################################################
# stats_SHS  : populated structure Stats_SHS 
# row_data   : data to append
#---------------------------------------------------------------------------------------------------------
# sheet_name     : append the row at the end of this sheet
# column_names   : name of the columns of the data 
# overwrite_last : if true, the appended row overwrites the last row in the file
##########################################################################################################
function save_results_SHS(stats_SHS, outfile; sheet_name="Sheet1")
    row_stats = [            
            stats_SHS.inst_name               
            stats_SHS.D
            stats_SHS.b
            ...
            stats_SHS.Φ
            stats_SHS.δ
            stats_SHS.weight_trivial
            stats_SHS.weight_nontrivial
            stats_SHS.weight_total
            stats_SHS.time_clustering
            stats_SHS.time_greedy
            stats_SHS.time_exact
            stats_SHS.n_nodes_BnC
            stats_SHS.n_integer_nodes_BnC
            stats_SHS.n_submodular_cuts
            stats_SHS.P
            stats_SHS.P1
            stats_SHS.P2
            stats_SHS.P1_auto
            stats_SHS.P1_weight_min
            stats_SHS.P1_weight
            string(stats_SHS.x_greedy)         
            stats_SHS.Z_greedy
            stats_SHS.Z_greedy_stoch
            string(stats_SHS.x_opt)                   
            stats_SHS.Z_opt
            stats_SHS.Z_opt_stoch
            stats_SHS.limit_time
            stats_SHS.is_optimal
    ]
    append_xl_row(outfile, row_stats; column_names=column_names_SHS, sheet_name=sheet_name)
end

##########################################################################################################
######################### Print important fields of Stats_SHS to standard output #########################
##########################################################################################################
# stats_SHS  : populated structure Stats_SHS 
##########################################################################################################
function print_results_SHS(stats_SHS)
    if(stats_SHS.P1_auto)
        println("Method            : SHS, Ω = Ω_i*")
    elseif(stats_SHS.time_clustering == 0)
        println("Method            : SAA")
    else
        println("Method            : SHS, Ω = ",stats_SHS.P1_weight_min)
    end
    println("Instance          : ",stats_SHS.inst_name)
    println("Optimal           : ",stats_SHS.is_optimal)
    println("Weight total      : ",stats_SHS.weight_total/stats_SHS.S)
    println("Weight nontrivial : ",stats_SHS.weight_nontrivial/stats_SHS.S)
    println("Value             : ",stats_SHS.Z_opt,
    " (",round(100*stats_SHS.S*stats_SHS.Z_opt/stats_SHS.weight_nontrivial, digits=1),"% of nontrivial), (",
         round(100*stats_SHS.S*stats_SHS.Z_opt/stats_SHS.weight_total, digits=1),"% of total)")
    println("Value (stoch)     : ",stats_SHS.Z_opt_stoch)
    println("H                 : ",stats_SHS.H)
    println("δ*                : ",stats_SHS.δ)
    println("P1                : ",stats_SHS.P1)
    println("P2                : ",stats_SHS.P2)
    println("n_nodes_BnC       : ",stats_SHS.n_nodes_BnC)
    println("time_exact        : ",stats_SHS.time_exact,"\n")
end