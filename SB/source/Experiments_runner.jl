                ...
                for S ∈ S_list
                    ...
                    for rep=1:n_rep
                        ...
                        for b ∈ b_list
                            ...
                            stats_SHS = init_Stats_SHS(D ; inst_name=inst_name, b=b, N=N, S=S, H=H, Φ=Φ, weight_trivial=weight_trivial, 
                            weight_nontrivial=weight_nontrivial, weight_total=weight_total, time_clustering=time_clustering, limit_time=limit_time)

                            # Solve the instance
                            stats_SHS = SHS(a, ω, b; P1_auto=P1_auto, min_P_submod=min_P_submod, tvd_min=tvd_min, P1_weight_min=P1_weight_min,
                             heuristic=heuristic, display=display, limit_time=limit_time, stats_SHS=stats_SHS)
                            
                            # Compute the objective value for the MNL problem
                            stats_SHS.Z_greedy_stoch = objective_value_MNL(stats_SHS.x_greedy,q,V,W_super)
                            stats_SHS.Z_opt_stoch = objective_value_MNL(stats_SHS.x_opt,q,V,W_super)

                            # Normalize the objective value of the simulation-based model according to the number of scenarios
                            stats_SHS.Z_greedy = stats_SHS.Z_greedy/S
                            stats_SHS.Z_opt    = stats_SHS.Z_opt/S

                            # Save the results in a file
                            save_results_SHS(stats_SHS, excel_file; sheet_name=sheet_name)

                            # Print basic information to standard output
                            if(verbose≥2) print_results_SHS(stats_SHS) end
                            flush(stdout)
                        end
                    end
                end
